...
## 🧪 测试与注意事项
- **标准测试用例**：使用了 [Figma Music Player UI Kit](https://www.figma.com/community/file/1230440663355118588) 进行开发测试。
- **🚨 致命操作警告**：导入 FGUI 后，**绝对禁止**直接点击编辑器内的"刷新"按钮。
  - **正确做法**：关闭项目 -> 重新打开。
  - **原因**：直接刷新会重置 `package.xml` 的资源 ID，导致资源引用全部错乱。这是不可接受的破坏性行为。
